...
import { diagnosisScenarios, } from "./diagnosis-scenarios";
const API_URL = process.env.DIAGNOSIS_API_URL ??
...
function evaluateScenario(scenario, apiResult) {
    ...
    const expectsNoDiagnosis = scenario.expectedResultStatus === "EXPECTED_NO_DIAGNOSIS" ||
...
function buildDiseaseMetrics(results) {
    const diseaseCodes = [
        "P001",
        "P002",
        "P003",
    ...
    return diseaseCodes.map((diseaseCode) => {
        const diagnosticResults = results.filter((result) => result.targetDiseaseCode !== null);
        const tp = diagnosticResults.filter((result) => result.targetDiseaseCode === diseaseCode &&
            result.predictedTop1 === diseaseCode).length;
        const fp = diagnosticResults.filter((result) => result.targetDiseaseCode !== diseaseCode &&
            result.predictedTop1 === diseaseCode).length;
        const fn = diagnosticResults.filter((result) => result.targetDiseaseCode === diseaseCode &&
            result.predictedTop1 !== diseaseCode).length;
...
function buildSummary(results) {
    const diagnosticResults = results.filter((result) => result.targetDiseaseCode !== null);
    const noDiagnosisResults = results.filter((result) => result.targetDiseaseCode === null);
    const top1Accuracy = divide(diagnosticResults.filter((result) => result.top1Correct).length, diagnosticResults.length);
    const top3Accuracy = divide(diagnosticResults.filter((result) => result.top3Correct).length, diagnosticResults.length);
    const urgencyAccuracy = divide(results.filter((result) => result.urgencyCorrect).length, results.length);
    const noDiagnosisAccuracy = divide(noDiagnosisResults.filter((result) => result.noDiagnosisCorrect).length, noDiagnosisResults.length);
    const passedRate = divide(results.filter((result) => result.passed).length, results.length);
    const diseaseMetrics = buildDiseaseMetrics(results);
    const macroPrecision = divide(diseaseMetrics.reduce((sum, item) => sum + item.precision, 0), diseaseMetrics.length);
    const macroRecall = divide(diseaseMetrics.reduce((sum, item) => sum + item.recall, 0), diseaseMetrics.length);
    const macroF1 = divide(diseaseMetrics.reduce((sum, item) => sum + item.f1, 0), diseaseMetrics.length);
    const byCategory = Object.fromEntries(Array.from(new Set(results.map((result) => result.category))).map((category) => {
        ...
        return [
            category,
            {
                ...
                passRate: divide(categoryResults.filter((result) => result.passed).length, categoryResults.length),
            },
        ];
    }));
    return {
...
function buildCsv(results) {
    ...
    const rows = results.map((result) => headers.map((header) => csvEscape(result[header])));
...
async function main() {
    ...
    for (const scenario of diagnosisScenarios) {
        ...
        console.log(`${icon} ${evaluation.scenarioId} | ${evaluation.category} | target=${evaluation.targetDiseaseCode ?? "-"} | top1=${evaluation.predictedTop1 ?? "-"} | urgency=${evaluation.urgencyActual ?? "-"}`);
    }
    ...
    await writeFile(path.join(REPORT_DIR, "diagnosis-evaluation.json"), JSON.stringify(jsonReport, null, 2), "utf8");
    await writeFile(path.join(REPORT_DIR, "diagnosis-evaluation.csv"), buildCsv(results), "utf8");
    console.log("\n=== Summary ===");
    ...
    console.log(`Urgency Accuracy: ${(summary.urgencyAccuracy * 100).toFixed(2)}%`);
    console.log(`No Diagnosis Accuracy: ${(summary.noDiagnosisAccuracy * 100).toFixed(2)}%`);
