import { diagnoseChild, getConsultationResultById, } from "../../inference/inference.service";
const DISCLAIMER = "Hasil ini adalah diagnosis awal dan bukan pengganti pemeriksaan dokter.";
export async function diagnosisRoutes(app) {
    app.post("/api/consultations/diagnose", async (request, reply) => {
        try {
            const result = await diagnoseChild(request.body);
