...
export function determineUrgency(params: {
  ...
  const redFlags = params.selectedSymptoms.filter(
    (symptom) => symptom.isRedFlag
  );

  const emergencySymptoms = params.selectedSymptoms.filter((symptom) =>
    emergencySymptomCodes.has(symptom.code)
  );

  const highRiskSymptoms = params.selectedSymptoms.filter((symptom) =>
    highRiskSymptomCodes.has(symptom.code)
  );
