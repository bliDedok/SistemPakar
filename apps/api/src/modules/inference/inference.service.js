...
import { KeepStatus, RuleOperator, SymptomRole, UrgencyMode, } from "../../generated/prisma/enums";
import { determineUrgency } from "./urgency.service";
import { generateRagExplanation, } from "../explanation/explanation.service";
import { calculateCfExpert, calculateCfPartial, combineCfValues, } from "./cf-helper";
...
function canAffectDiagnosis(weight) {
    return (weight.keepStatus !== KeepStatus.EXCLUDE &&
        weight.urgencyMode !== UrgencyMode.URGENCY_ONLY &&
        weight.symptomRole !== SymptomRole.CONTEXT_ONLY);
}
export async function diagnoseChild(payload) {
    const normalizedAnswers = payload.answers.filter((answer) => typeof answer.symptomCode === "string" &&
        answer.symptomCode.trim() !== "" &&
        typeof answer.userCf === "number" &&
        Number.isFinite(answer.userCf) &&
        answer.userCf >= 0 &&
        answer.userCf <= 1);
    const selectedAnswers = normalizedAnswers.filter((answer) => answer.userCf > 0);
    ...
    for (const disease of diseases) {
        const matchedRules = disease.rules.filter((rule) => {
            ...
            const matchedCount = detailSymptomIds.filter((symptomId) => selectedSymptomIds.has(symptomId)).length;
            const mandatorySymptoms = rule.details.filter((detail) => detail.isMandatory);
            const mandatoryOk = mandatorySymptoms.every((detail) => selectedSymptomIds.has(detail.symptomId));
            if (!mandatoryOk)
                return false;
        ...
        if (matchedRules.length === 0)
            continue;
        const matchedRuleSymptomIds = new Set(matchedRules.flatMap((rule) => rule.details.map((detail) => detail.symptomId)));
        ...
        if (calculationDetails.length === 0)
            continue;
        const cfFinal = round(combineCfValues(calculationDetails.map((item) => item.cfPartial)));
        if (cfFinal <= 0)
            continue;
        const matchedSymptoms = calculationDetails.map((detail) => ({
            symptomCode: detail.symptomCode,
            symptomName: detail.symptomName,
            role: detail.role,
        }));
        const diseaseRedFlags = [
            ...new Set(matchedWeights
                .filter((weight) => weight.symptom.isRedFlag ||
    ...
    const rankedResultsWithoutUrgency = resultCandidates
        .sort((a, b) => {
        if (b.cfFinal !== a.cfFinal)
            return b.cfFinal - a.cfFinal;
...
export async function getConsultationResultById(id) {
    ...
    const redFlags = [
        ...new Set(consultation.answers
            .filter((answer) => answer.symptom.isRedFlag)
            .map((answer) => answer.symptom.name)),
    ];
    const rankedResultsWithoutUrgency = consultation.results.map((item) => {
        const calculationDetails = safeJsonArray(item.calculationDetails);
        const matchedSymptoms = safeJsonArray(item.matchedSymptoms);
        const resultRedFlags = safeJsonArray(item.redFlags);
        const fallbackMatchedSymptoms = item.disease.weights
            .filter((weight) => consultation.answers.some((answer) => answer.symptomId === weight.symptomId && canAffectDiagnosis(weight)))
            .map((weight) => ({
            symptomCode: weight.symptom.code,
            symptomName: weight.symptom.name,
            role: String(weight.symptomRole),
        ...
        return {
            ...
            matchedSymptoms: matchedSymptoms.length > 0 ? matchedSymptoms : fallbackMatchedSymptoms,
            calculationDetails,
            supportingSymptoms: matchedSymptoms.length > 0
