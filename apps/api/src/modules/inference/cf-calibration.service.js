import { KeepStatus, SymptomRole, UrgencyMode, } from "../../generated/prisma/enums";
export function round(value, decimals = 4) {
    const multiplier = 10 ** decimals;
    return Math.round(value * multiplier) / multiplier;
}
export function combinePositiveCf(oldCf, newCf) {
    return oldCf + newCf * (1 - oldCf);
}
export function getDiagnosticRoleMultiplier(params) {
    if (params.keepStatus === KeepStatus.EXCLUDE)
        return 0;
    if (params.urgencyMode === UrgencyMode.URGENCY_ONLY)
        return 0;
...
export function combineEvidenceValues(values) {
    return round(values.reduce((current, next) => combinePositiveCf(current, next), 0));
}
