...
function estimateConfidence(message, alias) {
    const lower = normalize(message);
...
function inferChildName(message, current) {
    if (current)
        return current;
    const lower = normalize(message);
    const patterns = [
...
function mergeProfile(previous, message) {
    return {
...
function getMissingProfileFields(profile) {
    ...
    if (!profile.childName)
        missing.push("nama anak");
    if (profile.childAgeMonths == null)
        missing.push("usia anak dalam bulan");
    if (!profile.gender)
        missing.push("jenis kelamin anak");
    return missing;
}
async function getActiveSymptoms() {
...
function buildAliasIndex(symptoms) {
    return symptoms.map((symptom) => {
        ...
        const uniqueAliases = [...new Set(aliases)].filter((alias) => alias.length >= 3);
        return {
            symptom,
...
function extractSymptomsFromMessage(message, symptoms) {
    const lower = normalize(message);
...
function buildReply(args) {
    ...
    if (missingFields.length > 0) {
        return {
            reply: `Baik, saya bantu catat dulu. Mohon lengkapi ${missingFields.join(", ")} agar konsultasi bisa diproses dengan lebih rapi.`,
            missingFields,
            canDiagnose: false,
        };
    }
    ...
    if (args.cumulativePositiveCount === 0) {
        return {
            reply: "Saya belum menemukan gejala yang cocok dari konteks yang ada. Coba sebutkan gejalanya lebih spesifik, misalnya batuk, demam, pilek, napas cepat, muntah, atau diare.",
            missingFields,
            canDiagnose: false,
        };
    }
    ...
    return {
        reply: "Baik, data gejala sebelumnya masih saya simpan. Kalau ada keluhan tambahan, silakan lanjutkan. Data saat ini sebenarnya sudah bisa diproses ke diagnosis awal.",
        missingFields,
        canDiagnose: true,
    };
}
export async function processChatbotMessage(payload) {
    ...
    if (process.env.GEMINI_API_KEY) {
        try {
            ...
            const structuredSymptoms = llmResult.symptoms
            ...
            const negativeSymptoms = llmResult.negativeSymptoms
                .filter((item) => validCodes.has(item.code))
                .map((item) => ({
                code: item.code,
                symptomName: item.symptomName,
            ...
            const mergedProfile = {
                ...
                childAgeMonths: llmResult.profile.childAgeMonths ?? payload.profile?.childAgeMonths ?? null,
            ...
            const cumulativePositiveCount = Array.from(mergedSymptomMap.values()).filter((value) => value > 0).length;
