...
type DiagnosisResultForExplanation = {
  ...
  // field baru dari inference service
  cfFinal?: number;
  cfPercent?: number;
...
function buildEvidenceSourceNames(
  retrievedEvidence: {
    sourceName: string | null;
  }[]
) {
  ...
  if (!names.length) return "sumber evidence yang tersedia";
...
function buildTemplateExplanation(args: {
  ...
  const supportingSymptoms =
  top.supportingSymptoms?.length
    ? top.supportingSymptoms
    : top.matchedSymptoms
        ?.map((symptom) => symptom.symptomName)
        .filter((name): name is string => Boolean(name)) ?? [];

const supportingSymptomsText =
...
const symptomNames = args.results.flatMap((item) => {
  const supportingSymptoms =
    item.supportingSymptoms?.filter(
  ...
  const matchedSymptoms =
    item.matchedSymptoms
      ?.map((symptom) => symptom.symptomName)
      .filter(
        (symptomName): symptomName is string =>
          typeof symptomName === "string" && symptomName.trim().length > 0
      ) ?? [];
...
const top1Symptoms: string[] =
  top1?.supportingSymptoms?.length
    ? top1.supportingSymptoms.filter(
        (symptom): symptom is string =>
          typeof symptom === "string" && symptom.trim().length > 0
      )
...
const top1CfPercent = top1?.cfPercent ?? top1?.percentage ?? 0;
  ...
  const evidenceBasedExplanation =
